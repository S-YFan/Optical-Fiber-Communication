...
for eye_i in eye1:
    plt.plot(t, eye_i)
plt.title("Laser Modulated Eye Diagram (P)")
plt.xlabel("Time(ns)")
plt.xlim(-0.1,0.1)
plt.ylabel("Power")
plt.grid(True)
...
arg_50 = (np.pi / 4) * np.cos(omega_cl * t_carver + np.pi) + (np.pi / 4)
arg_33 = (np.pi / 2) * np.cos((omega_cl * t_carver+ np.pi) / 2)
arg_67 = (np.pi / 2) * np.cos(omega_cl * t_carver / 2) + (np.pi / 2)
#A_50_RZ = np.cos(2*np.pi*(np.cos(omega_cl*t_carver+np.pi))/7+2*np.pi/7)*np.exp(1j*(3*np.pi*(np.cos(omega_cl*t_carver+np.pi))/14+3*np.pi/14)) * E_output
#A_33_RZ = np.cos(4*np.pi*np.cos((omega_cl*t_carver+np.pi)/2)/7)*np.exp(1j*(3*np.pi*(np.cos((omega_cl*t_carver+np.pi)/2)/7))) * E_output
#A_67_RZ = np.cos(4*np.pi*np.cos(omega_cl*t_carver/2)/7+4*np.pi/7)*np.exp(1j*(3*np.pi*(np.cos((omega_cl*t_carver)/2)+1)/7)) * E_output
A_50_RZ = np.cos(arg_50) * np.exp(1j * (0.75 * arg_50)) * E_output
A_33_RZ = np.cos(arg_33) * np.exp(1j * (0.75 * arg_33)) * E_output
A_67_RZ = np.cos(arg_67) * np.exp(1j * (0.75 * arg_67)) * E_output
...
for eye_i in eye1:
    plt.plot(t, eye_i)
plt.title("Modulated eye diagram, 50%RZ")
plt.xlabel("Time(ns)")
plt.xlim(-0.1,0.1)
plt.ylabel("Power")
plt.grid(True)
...
for eye_i in eye1:
    plt.plot(t, eye_i)
plt.title("Modulated eye diagram, 33%RZ")
plt.xlabel("Time(ns)")
plt.xlim(-0.1,0.1)
plt.ylabel("Power")
plt.grid(True)
...
for eye_i in eye1:
    plt.plot(t, eye_i)
plt.title("Modulated eye diagram, 67%RZ")
plt.xlabel("Time(ns)")
plt.xlim(-0.1,0.1)
plt.ylabel("Power")
plt.grid(True)
